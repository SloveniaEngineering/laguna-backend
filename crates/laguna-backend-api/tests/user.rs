...
async fn test_delete_inexistant_user() {
    ...
    let delete_me_res = common::as_logged_in(
        access_token,
        refresh_token,
        TestRequest::delete().uri(&format!("/api/user/delete/{}", Uuid::new_v4())),
        &app,
    )
    .await
    .unwrap();
    assert_eq!(delete_me_res.status(), StatusCode::NOT_FOUND);
    common::teardown(pool, database_url).await;
}
...
async fn test_get_inexistant_user() {
    ...
    let get_me_res = common::as_logged_in(
        access_token,
        refresh_token,
        TestRequest::with_uri(&format!("/api/user/{}", Uuid::new_v4())),
        &app,
    )
    .await
    .unwrap();
    assert_eq!(get_me_res.status(), StatusCode::NOT_FOUND);
    common::teardown(pool, database_url).await;
}
